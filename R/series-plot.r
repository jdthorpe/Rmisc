...
#' @param m.side see \code{link[base]{plot}} for details
#' @param fun optional custom plotting function
#' @param ... additional parameters to \code{fun} (if provided), else 
#' additional arguments to \code{\link[graphics]{text}} if the parameter 
#' \code{labels} is specified.  Otherwise, additional atrguments to 
...
#' pch = sample(LETTERS,m*n,replace=TRUE)
...
#' 			pch = pch,
#' 			cex = 1.2,
#' 			col = c('black','brown3','dodgerblue3'),
#' 			bg = c(NA,'gray55'),
#' 			main ='foo',
#' 			xlab = 'Value',
#' 			ylab = 'ID') 
...
#' 
#' # --------------------------------------------------
#' # an ugly but illustrative use of 'add = TRUE'
#' # --------------------------------------------------
#' plot.serial(x = x,
#' 			id = id,
#' 			type='b',
...
plot.serial <- function(x,y,id,
	...
	if(missing(x)){
		...
		if(missing(ylim))
			ylim <- range(if(!identical(grep('y',log),integer(0))) log(values) else values,na.rm=TRUE)
		if( (!identical(grep('y',log),integer(0))) & (min(values, na.rm = TRUE) <= 0))
		...
		if(missing(xlim))
			xlim <- range(if(!identical(grep('x',log),integer(0))) log(values) else values,na.rm=TRUE)
		if( (!identical(grep('x',log),integer(0))) & (min(values, na.rm = TRUE) <= 0))
			message('WARNING: negative values and zeros are excluded from plot.')
	}
	#SETUP THE PLOT
	if(!add){
