...
#' @param specificity.bias='over'
#' @param case.values
#' @param sensitivity.bias='proportional'
...
THRESHOLD <- function(x
	...
	# if there are cases which lie between the upper and lower bounds for the
...
##########################################################################################
	...
	if( (length(case.values) > 0)
		...
		#return(thresholds[[sensitivity.bias]])
		if(verbose)
			print(thresholds)
		return.val <- getconf(thresholds[[sensitivity.bias]])
		return(return.val)
...
ecdf2 <- function(x,ref
					,interpMethod='proportional'
					,na.rm = FALSE
					){
	if(missing(ref))ref <- x# an optional reference group
	...
	if(!diff(boundary)){
		...
		if(boundary[1] == max(x)){
			...
			if(!quietly)
				warning('Threshold is higher than all observed values in the controls')
			return( (min(cases[cases > (boundary[1])]) + (boundary[1]))/2 )
		}

		#if the specificity is below the expected value for the highest order statistic
		#DESIRED THRESHOLD IS BLOW THE CONTROLS
		if(boundary[1] == min(x)){
			if( min(cases) >= (boundary[1])){
				if(!quietly)
					warning('Threshold is the lowest value among all observations')
				return(boundary[1])
			}
			if(!quietly)
				warning('Threshold is lower than all observed values in the controls')
			return( (max(cases[cases < (boundary[1])]) + (boundary[1]))/2)
		}
	...
	# The desired threshold lies between two controls with discinct values.
	# Check if there are any cases between the boundary controls and if so
	# Choose an appropriate threshold WRT the distribution of those cases.

	#-- print(boundary)
	#-- print(sum(( (cases > boundary[1])
	#-- 	     & (cases < boundary[2]))))
	if( (length(cases) > 0)
		& any( (cases > boundary[1])
		     & (cases < boundary[2]))){

		#warning('	cases lie between the boundaries')
		#get the boundary points (controls) and the cases between the boundaries
		pts <- sort(c( boundary[1]
					, cases[(cases > (boundary[1]))
						   &(cases < (boundary[2]))]
					, boundary[2]
					))
		if(length(pts) <=2)stop('error in selecting cases between boundaries')
		(midpoints <- ((pts[-1] + pts[-length(pts)] )/2))
		...
		# if the mean would give a threshold equal to observed value, default to the median
		if( out[['mean']] %in% cases)out[['mean']] <- out[['median']]

		if(any(unlist(out) %in% boundary))stop("Error in 'threshold'")
		return(out[[sensitivity.bias]])
	}
	# easy peasy: no cases between the controls and no ties at the specificity level
