...
#' @param xlab,ylab x and y axis lables. 
...
#' @param xlim limitis of the x axis.  See \code{link[graphics]{plot.default}} for details.
...
#' @param kernel kernal type (devault = 'c'; see \code{\link[stats]{density}} for details
#' @param bandwidth bandwidth smoothing parameter. See \code{\link[stats]{density}} for details.
#' @param adjust adjustment parmeter. See \code{\link[stats]{density}} for details.
					...
					...)
{
	...
	if(!length(border) %in% c(length(labels),1)  )
		border = rep_len(border,length(border))
	if(!length(col) %in% c(length(labels),1)  )
		col = col(border,length(border))
	if( length(border) == 1)
	...
	# this version won'tRUE complain if everything is missing...
	...
	if( (ylog) & (min(y, na.rm = TRUE) <= 0))
		warning('negative and zero values are excluded from plot')

	if( (ylog) & (max(y, na.rm = TRUE) <= 0)) {
		warning('cannot plot negative valus on logrithmic scale')
		log = ''
	}

	if(ylog)
		ymin = range(y[y>0 & is.finite(y)], na.rm = TRUE,finite = TRUE)[1]
	else
		ymin = range(y, na.rm = TRUE,finite = TRUE)[1]

	# PREPARE THE PLOTTING REGION
	plot(x = if(length(labels) == 2) c(.6,2.4) else c(.8,length(labels) + .2),
		 y = c(ymin,range(y, na.rm = TRUE,finite = TRUE)[2]),
	...
	if(!identical(bg.panel, 'white')) {
		...
		if(length(bg.panel) != length(labels))
			bg.panel <- rep_len(bg.panel,length(labels))
		for(i in 1:length(labels)) {
			polygon( x = c(i - .5,i - .5,i + .5,i + .5)
					,y = c(	 min(y,na.rm = TRUE) - abs( max(y,na.rm = TRUE) - min(y,na.rm = TRUE))
							,max(y,na.rm = TRUE) + abs( max(y,na.rm = TRUE) - min(y,na.rm = TRUE))
							,max(y,na.rm = TRUE) + abs( max(y,na.rm = TRUE) - min(y,na.rm = TRUE))
							,min(y,na.rm = TRUE) - abs( max(y,na.rm = TRUE) - min(y,na.rm = TRUE))
							)
					,col = bg.panel[i]
					,border = NA)
		}
