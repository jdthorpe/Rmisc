...
#' Pointers to a temporary files
#' 
#' Pointers to a temporary files.
#' 
#' @rdname Temporary-Files
#' @export
...
#' Test if a string is missing or contains only whitespace
#' 
#' Test if a string is missing or contains only whitespace.
#' 
#' @export
#' @param x A character vetor to test for missing or emtpy strings.
empty <- function (x) 
	(is.na(x) | grepl("^[ \t\n]*$", x))
...
#' @param AddNames logical. Should the varialbe names be aded to the dim names.
#' @export
#' @examples
#' n = 1000
