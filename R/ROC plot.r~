...
')
		...
		p.value <- wilcox.test(x = x[cases], y = x[!cases])$p.value
		...
		if(print.auc){
			...
			cAUC <- paste('AUC = ', tochar(auc,digits.auc), ' (p ', p.value.text, ')', sep = '')
			text(print.loc$x,
				print.loc$y - yoffset,
				cAUC,
				col = print.col,
				cex = cex.label,
				pos = 4)
			yoffset <- yoffset + 1.2*strheight(cAUC,cex = cex.label)
		}
		...
		if(length(spec)>0) {
			...
			text(x = print.loc$x + cwd,
				 ...
				 cex = cex.text,
				 pos = 4)
			yoffset <- yoffset + 1.2*strheight('Specificity',cex = cex.text)
			for(r in 1:length(spec)) {
				...
				text(x = print.loc$x + cwd,
					 ...
					 cex = cex.text,
					 pos = 4)
				yoffset <- yoffset + 1.2*strheight(tochar(temp[r,2],3),cex = cex.text)
			}
		}
	}
	out <- list(auc = auc
